...
public class GetController
{
     ...
     * @return
     */
    @RequestMapping(value = "/common/get/{param}/{paramTwo}", method = RequestMethod.GET)
    public String commonGetWithTwoParam(HttpServletRequest request, HttpServletResponse response, @RequestBody(required = false) String parameters)
    {
        try
        {
            logger.info("------开始打印带两个动态参数的GET消息头信息------");
            ...
            logger.info("------打印带两个动态参数的GET消息头信息结束------");

            //当报文为application/json时参数从parameters中获取，当报文为application/x-www-form-urlencoded时参数从request中获取
            logger.info("------开始打印带两个动态参数的GET请求报文------");

            String params = request.getServletPath().replace("/m2m/common/get", "").replaceAll("/", "").replaceAll("\\{", "").replaceAll("}", ",");
            logger.info("url中/后请求参数:" + params.substring(0, params.length() - 1));
            logger.info("url中?后请求参数:" + JSONObject.fromObject(request.getParameterMap()).toString());
            logger.info("消息体中请求参数:" + parameters);

            logger.info("------打印带两个动态参数的GET请求报文结束------");
        } catch (JSONException e)
        {
            throw new RuntimeException("字符串转json出错");
        } catch (Exception e)
